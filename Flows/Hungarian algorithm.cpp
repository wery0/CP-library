...
pair<CostSum, vector<int>> hungarian_algorithm(const vector<vector<Cost>>& cost) {
    ...
    for (int it = 0; it < 2; ++it) {
        for (int row = 0; row < V; ++row) {
            ...
            auto u2 = numeric_limits<CostSum>::max();
            int c1 = 0;
            for (int c = 0; c < V; ++c) {
                auto u = residual(row, c);
                if (u < u1 || u == u1 && col_mate[c1] != -1) u2 = u1, u1 = u, c1 = c;
                else if (u < u2) u2 = u;
            }
            if (u1 < u2) pi[c1] -= u2 - u1;
            if (int r1 = col_mate[c1]; r1 != -1) row_mate[r1] = col_mate[c1] = -1;
            row_mate[row] = c1;
            col_mate[c1] = row;
        }
    }
    ...
    for (int row = 0; row < V; ++row) {
        ...
        while (true) {
            ...
            int c1 = cols[scanned++], r1 = col_mate[c1];
