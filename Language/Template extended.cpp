...
template<typename T> vec<vec<int>> get_cycles_of_perm(vec<T> &m, int index) {int a = m.size(); vec<vec<int>> ans; vec<bool> us(a); for (int q = 0; q < a; ++q) {if (us[q]) continue; vec<int> tyt; for (int w = q; !us[w]; w = m[w] - index) {tyt.pb(w); us[w] = 1;} ans.pb(tyt);} return ans;}
