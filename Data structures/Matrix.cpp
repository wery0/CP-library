...
#pragma GCC unroll 16
    ...
    //Returns {# of solutions (2 if inf), one of solutions itself if exists}. O(min(n, m) * nm)
    pair<int, vector<T>> solve_SLA(T EPS = is_integral_v<T> ? 0 : 1e-12) const {assert(m > 1); auto u = t; auto eq = [&](T x, T y) {return abs(x - y) <= EPS;}; for (size_t i = 0, j = 0; i < n && j + 1 < m; ++i, ++j) {size_t r = i; while (r < n && eq(u[r][j], ZERO)) ++r; if (r == n) {--i; continue;} if (r != i) swap(u[i], u[r]); normalize_row(u[i], j); for (size_t k = 0; k < n; ++k) {if (k != i) add_row(u[i], u[k], j, -u[k][j]);}} for (const auto& row : u) {if (all_of(row.begin(), row.end() - 1, [&](T x) {return eq(x, ZERO);}) && !eq(row[m - 1], ZERO)) return {0, {}};} vector<T> solution(m - 1); size_t cnt = 0; for (size_t i = 0, j = 0; i < n; ++i, ++j) {while (eq(u[i][j], ZERO) && j + 1 < m) ++j; if (j + 1 >= m) break; solution[j] = u[i][m - 1]; ++cnt;} return {1 + (cnt != solution.size()), solution};}
