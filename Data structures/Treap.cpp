...
private:
    ...
    pair<Node*, Node*> split_size(Node* n, ll k) {
        if (!n) return {0, 0};
        push(n);
        if (k <= gsz(n->l)) {
            pair<Node*, Node*> p = split_size(n->l, k);
            n->l = p.S; upd(n);
            p.S = n;
            return p;
        }
        pair<Node*, Node*> p = split_size(n->r, k - gsz(n->l) - 1);
        n->r = p.F; upd(n);
        p.F = n;
        return p;
    }

    pair<Node*, Node*> split_key(Node* n, K k) {
        if (!n) return {0, 0};
        push(n);
        if (k < n->x) {
            pair<Node*, Node*> p = split_key(n->l, k);
            n->l = p.S; upd(n);
            p.S = n;
            return p;
        }
        pair<Node*, Node*> p = split_key(n->r, k);
        n->r = p.F; upd(n);
        p.F = n;
        return p;
    }
    ...
    Node* insert_node(Node* n, Node* nw) {
        push(n);
        if (!n || nw->y > n->y) {
            pair<Node*, Node*> p = split_key(n, nw->x);
            nw->l = p.F;
            nw->r = p.S;
            upd(nw);
            return nw;
...
public:
    ...
    void insert_key_at_pos(int pos, K x, V val = UNDEF) {pair<Node*, Node*> p = split_size(root, pos); root = merge(merge(p.F, new Node(x, val)), p.S);}
    ...
    vector<K> get_keys_from_seg(int l, int len) {vector<K> res; get_keys_on_subsegment(root, l, len, res); get_keys_on_subsegment(l, len, res); return res;}

    void print_keys(str end_string = "") {print_keys(root); cout << end_string;}
    void print_vals(str end_string = "") {print_vals(root); cout << end_string;}
};