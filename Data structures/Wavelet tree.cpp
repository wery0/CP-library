template<typename T, bool do_compress>
class wavelet_tree {
...
public:
    ...
    template<typename I>
    wavelet_tree(I first, I last): n(std::distance(first, last)) {
        if (!n) return;
        ...
        function<void(int, int, int, T, T)> build = [&](int layer, int l, int r, T vl, T vr) {
            if (l > r || vl == vr) return;
            T vm = vl + (vr - vl) / 2;
            for (int i = l; i <= r; ++i) if (m[i] > vm) bvs[layer].set(i);
            int p = stable_partition(m.begin() + l, m.begin() + r + 1, [vm](T x) {return x <= vm;}) - m.begin();
    ...
    //O(log(# distinct)) if do_compress == true, O(log(mxe - mne)) otherwise
    int seg_count_leq(int ql, int qr, T x) {
        assert(ql <= qr && qr < n);
        if (do_compress) {
        ...
        res += vl == vr && vl <= x && ql <= qr ? qr - ql + 1 : 0;
        return res;
    }
