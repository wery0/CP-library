...
//Stress test: https://pastebin.com/qrb6wpvH
...
class custom_bitset {
 ...
 public:
	...
	void reset(size_t l, size_t r) {
		if (l / BIT == r / BIT) {
			m[l >> LOG] &= z[l & BITM][r & BITM];
			return;
		}
		m[l >> LOG] &= z[l & BITM][BITM];
		m[r >> LOG] &= z[0][r & BITM];
