...
class hashmap_open_addressing {
	...
	struct Iterator : public std::forward_iterator_tag {
		...
		Iterator(size_t pos, const vector<pair<K, V>>& store, const vector<char>& is_occupied): pos_(pos), store_(store), is_occupied_(is_occupied) {
	...
	void rehash(size_t new_capacity) {
		...
		vector<pair<K, V>> content; content.reserve(size_);
		...
		for (const auto& [key, value] : content) insert(key, value);
	}

	size_t size_;
    size_t capacity_;
    vector<pair<K, V>> store_;
    vector<char> is_occupied_;
    double load_factor_ = 0.6;

public:
    hashmap_open_addressing(size_t capacity = 1): size_(0) {
    	assert(capacity > 0);
    ...
    bool contains(const K& key) const {
        ...
        while (is_occupied_[pos]) {
            if (store_[pos].first == key) return true;
            next_pos(pos);
        }
        return false;
    }
    ...
    void insert(const K& key, const V& value) {
        size_t pos = hsh(key);
        while (is_occupied_[pos]) {
            if (store_[pos].first == key) return;
            next_pos(pos);
        }
        store_[pos] = {key, value};
    ...
    const V& at(const K& key) const {
    	size_t pos = hsh(key);
    	while (is_occupied_[pos]) {
            if (store_[pos].first == key) return store_[pos].second;
            next_pos(pos);
        }
        assert(0);
        return V();
    ...
    V& operator[](const K& key) {
        if (size_ + 1 >= capacity_ * load_factor_) {
        	rehash(capacity_ * 2);
        }
    	size_t pos = hsh(key);
    	while (is_occupied_[pos]) {
        ...
        store_[pos] = {key, V()};
        is_occupied_[pos] = true;
        return store_[pos].second;
    }

    void erase(const K& key) {
		size_t i = hsh(key);
		while (is_occupied_[i]) {
			if (store_[i].first == key) break;
			next_pos(i);
		}
