...
public:
    ...
    int count_connected_components(int min_component_size = 1, bool count_single_vertex_components_with_self_edges = false) {assert(is_prepared); vector<char> us(V), has_self_edge(V); int cnt = 0; function<void(int)> dfs = [&](int v) -> void {us[v] = 1; ++cnt; for (const auto& h : (*this)[v]) {if (v == h.to) has_self_edge[v] = 1; if (!us[h.to]) dfs(h.to);}}; int ans = 0; for (int v = 0; v < V; ++v, cnt = 0)if (!us[v]) dfs(v), ans += cnt >= min_component_size || count_single_vertex_components_with_self_edges && cnt == 1 && has_self_edge[v]; return ans;}
    int count_weakly_connected_components(int min_component_size = 1, bool count_single_vertex_components_with_self_edges = false) {assert(is_prepared); vector<int> pr(V), sz(V, 1); vector<char> has_self_edge(V); iota(pr.begin(), pr.end(), 0); function<int(int)> find = [&](int v) -> int {return pr[v] = v == pr[v] ? v : find(pr[v]);}; for (int v = 0; v < V; ++v) {for (int i = fir[v]; i < fir[v + 1]; ++i) {const auto& e = store[i]; if (v == e.to) has_self_edge[v] = 1; int pv = find(v), ph = find(e.to); if (pv != ph) {if (sz[pv] > sz[ph]) swap(pv, ph); sz[ph] += sz[pv]; pr[pv] = ph;}}} int ans = 0; for (int v = 0; v < V; ++v) ans += pr[v] == v && (sz[v] >= min_component_size || count_single_vertex_components_with_self_edges && sz[v] == 1 && has_self_edge[v]); return ans;}
    ...
    bool is_bipartite() {assert(is_prepared); vector<int> col(V, -1); bool fl = 1; function<void(int, int)> dfs = [&](int v, int c) -> void {col[v] = c; for (const auto& h : (*this)[v]) {if (col[h.to] == -1) dfs(h.to, c ^ 1); else fl &= col[h.to] != c;}}; for (int q = 0; q < V; ++q) if (col[q] == -1) dfs(q, 0); return fl;}
    ...
    bool is_acyclic_directed() {assert(is_prepared); vector<int> state(V); function<bool(int)> dfs = [&](int v) -> bool {state[v] = 1; for (const auto& h : (*this)[v]) {if (state[h.to] == 1) return false; if (state[h.to] == 0 && !dfs(h.to)) return false;} state[v] = 2; return true;}; for (int q = 0; q < V; ++q) {if (state[q] == 0 && !dfs(q)) return false;} return true;}
    bool is_acyclic_undirected() {assert(is_prepared); vector<int> state(V); function<bool(int, int)> dfs = [&](int v, int p) -> bool {state[v] = 1; int cnt_par = 0; for (const auto& h : (*this)[v]) {if (h.to == p) {if (++cnt_par == 1) continue;} if (state[h.to] == 1) return false; if (state[h.to] == 0 && !dfs(h.to, v)) return false;} state[v] = 2; return true;}; for (int q = 0; q < V; ++q) if (state[q] == 0 && !dfs(q, -1)) return false; return true;}
    ...
    vector<int> get_topsort() {assert(is_acyclic_directed()); vector<bool> us(V); vector<int> ans; ans.reserve(V); function<void(int)> dfs = [&](int v) -> void {us[v] = 1; for (const auto& e : (*this)[v]) {if (!us[e.to]) dfs(e.to);} ans.push_back(v);}; for (int q = 0; q < V; ++q) {if (!us[q]) dfs(q);} reverse(ans.begin(), ans.end()); return ans;}
    ...
    vector<int> get_bridges_as_edge_nums() {assert(is_prepared); vector<bool> is_bridge(E), us(V); vector<int> tin(V), fup(V); function<int(int, int)> dfs = [&](int v, int p) -> int {static int T = 0; us[v] = 1; tin[v] = fup[v] = T++; int cnt_par = 0, ie = -1; for (int i = fir[v]; i < fir[v + 1]; ++i) {const auto& h = store[i]; if (h.to == p) {if (++cnt_par > 1) {fup[v] = min(fup[v], tin[h.to]);} else ie = i;} else if (!us[h.to]) {int inv_edge = dfs(h.to, v); fup[v] = min(fup[v], fup[h.to]); is_bridge[i] = is_bridge[inv_edge] = fup[h.to] > tin[v];} else fup[v] = min(fup[v], tin[h.to]);} return ie;}; for (int q = 0; q < V; ++q) if (!us[q]) dfs(q, -1); int cnt_bridges = count(is_bridge.begin(), is_bridge.end(), true) / 2; vector<int> ans(cnt_bridges); for (int q = 0, j = 0; q < V; ++q) {for (int i = fir[q]; i < fir[q + 1]; ++i) {const auto& h = store[i]; if (is_bridge[i] && q < h.to) ans[j++] = h.data;}} return ans;}
    vector<int> get_bipartite_coloring() {assert(is_prepared); vector<int> col(V, -1); bool fl = 1; function<void(int, int)> dfs = [&](int v, int c) -> void {col[v] = c; for (const auto& h : (*this)[v]) {if (col[h.to] == -1) dfs(h.to, c ^ 1); else fl &= col[h.to] != c;}}; for (int v = 0, p = 1; v < V && fl; ++v) if (col[v] == -1) dfs(v, p ^= 1); assert(fl && "Graph is not bipartite"); return col;}
    vector<vector<int>> get_connected_components_undirected() {assert(is_prepared); vector<vector<int>> ans; vector<bool> us(V); function<void(int)> dfs = [&](int v) -> void {us[v] = 1; ans.back().push_back(v); for (const auto& e : (*this)[v]) {if (us[e.to]) continue; dfs(e.to);}}; for (int q = 0; q < V; ++q) {if (us[q]) continue; ans.push_back({}); dfs(q);} return ans;}
    vector<vector<int>> get_vertex_biconnected_components() {assert(is_prepared); vector<bool> us(V); vector<int> tin(V), fup(V), stq; vector<vector<int>> ans; ans.reserve(V); stq.reserve(V); function<void(int, int)> dfs = [&](int v, int p) -> void {static int T = 0; stq.push_back(v); us[v] = 1; tin[v] = fup[v] = T++; int chd = 0; for (const auto& h : (*this)[v]) {if (h.to == p) continue; if (!us[h.to]) {++chd; int stop_sz = (int)stq.size(); dfs(h.to, v); fup[v] = min(fup[v], fup[h.to]); if ((p == -1 && chd > 1) || (p != -1 && fup[h.to] >= tin[v])) {ans.push_back({v}); ans.back().reserve(stq.size() - stop_sz + 1); for (; (int)stq.size() > stop_sz;) {ans.back().push_back(stq.back()); stq.pop_back();}}} else fup[v] = min(fup[v], tin[h.to]);}}; for (int q = 0; q < V; ++q) {if (!us[q]) {dfs(q, -1); ans.push_back(stq); stq.clear();}} return ans;}
    vector<vector<int>> get_edge_biconnected_components() {assert(is_prepared); vector<bool> is_bridge(E), us(V); vector<int> tin(V), fup(V); function<int(int, int)> dfs = [&](int v, int p) -> int {static int T = 0; us[v] = 1; tin[v] = fup[v] = T++; int cnt_par = 0, ie = -1; for (int i = fir[v]; i < fir[v + 1]; ++i) {const auto& h = store[i]; if (h.to == p) {if (++cnt_par > 1) {fup[v] = min(fup[v], tin[h.to]);} else ie = i;} else if (!us[h.to]) {int inv_edge = dfs(h.to, v); fup[v] = min(fup[v], fup[h.to]); is_bridge[i] = is_bridge[inv_edge] = fup[h.to] > tin[v];} else fup[v] = min(fup[v], tin[h.to]);} return ie;}; int cnt_cmp = 0; for (int q = 0; q < V; ++q) if (!us[q]) dfs(q, -1), ++cnt_cmp; int cnt_bridges = count(is_bridge.begin(), is_bridge.end(), true) / 2; int cnt_tec_cmp = cnt_cmp + cnt_bridges; vector<vector<int>> ans(cnt_tec_cmp); fill(us.begin(), us.end(), false); function<void(int, int)> go = [&](int v, int nc) -> void {us[v] = 1; assert(nc < cnt_tec_cmp); ans[nc].push_back(v); for (int i = fir[v]; i < fir[v + 1]; ++i) {const auto& h = store[i]; if (!us[h.to] && !is_bridge[i]) go(h.to, nc);}}; for (int q = 0, nc = 0; q < V; ++q) if (!us[q]) go(q, nc++); return ans;}
    vector<int> calc_connected_components_after_vertex_removal() {assert(is_prepared); vector<bool> us(V); vector<int> ans(V, V > 1), tin(V), fup(V); function<void(int, int)> dfs = [&](int v, int p) -> void {static int T = 0; us[v] = 1; tin[v] = fup[v] = T++; int chd = 0; for (const auto& h : (*this)[v]) {if (h.to == p) continue; if (!us[h.to]) {++chd; dfs(h.to, v); fup[v] = min(fup[v], fup[h.to]); ans[v] += p == -1 ? chd > 1 : fup[h.to] >= tin[v];} else fup[v] = min(fup[v], tin[h.to]);}}; for (int v = 0; v < V; ++v) if (!us[v]) dfs(v, -1); return ans;}
