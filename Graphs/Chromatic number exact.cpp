...
class ChromaticNumberExact {
    ...
    static auto duration_milliseconds(auto a) {return chrono::duration_cast<chrono::milliseconds>(a).count();}
...
public:
    ...
    //If it doesn't finish, set positive time limit (in seconds)
    pair<int, vector<int>> calc_chromatic_number_and_coloring(const double TL = 0) const {
        ...
        function<void(int, int)> go = [&](int mx, int eso) {
            ...
            die |= !++cnt && TL > 0 && duration_milliseconds(time_stamp() - start_time) / 1e3 > TL;
            if (eso == 0) {
