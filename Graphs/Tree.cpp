...
public:
    ...
    int calc_diameter_unweighted() {assert(is_prepared); int v = (int)(max_element(dep.begin(), dep.end()) - dep.begin()); function<int(int, int)> dfs = [&](int v, int p) -> int {int mxd = -1; for (const auto& e : g[v]) {if (e.to != p) {int tyt = dfs(e.to, v); mxd = max(mxd, tyt);}} return mxd + 1;}; return dfs(v, -1);}
    vector<int> calc_euler_traversal() {assert(is_prepared); vector<int> eul(V); int T = 0; function<void(int, int)> dfs = [&](int v, int p) -> void {eul[T++] = v; for (const auto& e : g[v]) {if (e.to == p) continue; dfs(e.to, v);}}; dfs(root, -1); return eul;}
    ...
    vector<int> calc_foreach_furthest_vertex_unweighted() {assert(is_prepared); vector<int> deepest1(V), deepest2(V, -1); function<int(int, int)> dfs = [&](int v, int p) -> int {deepest1[v] = v; for (const auto& e : g[v]) {if (e.to == p) continue; int tyt = dfs(e.to, v); if (dep[tyt] >= dep[deepest1[v]]) {deepest2[v] = deepest1[v]; deepest1[v] = tyt;} else if (dep[tyt] > dep[deepest2[v]]) {deepest2[v] = tyt;}} return deepest1[v];}; dfs(root, -1); vector<int> furthest(V); function<void(int, int, int, int)> go = [&](int v, int u, int dst_to_u, int p) -> void {furthest[v] = dst_to_u > dep[deepest1[v]] - dep[v] ? u : deepest1[v]; for (const auto& e : g[v]) {if (e.to == p) continue; int nwu = deepest1[e.to] == deepest1[v] ? deepest2[v] : deepest1[v]; int nwd = nwu == -1 ? -1 : dep[nwu] - dep[v]; if (nwu == -1 || dst_to_u > dep[nwu] - dep[v]) nwu = u, nwd = dst_to_u; go(e.to, nwu, nwd + 1, v);}}; go(root, -1, -1, -1); return furthest;}
    template<typename D> vector<int> calc_foreach_furthest_vertex_weighted() {assert(is_prepared); vector<int> deepest1(V), deepest2(V, -1); vector<D> wdep(V); function<int(int, int)> dfs = [&](int v, int p) -> int {deepest1[v] = v; for (const auto& e : g[v]) {if (e.to == p) continue; wdep[e.to] = wdep[v] + e.data; int tyt = dfs(e.to, v); if (wdep[tyt] >= wdep[deepest1[v]]) {deepest2[v] = deepest1[v]; deepest1[v] = tyt;} else if (wdep[tyt] > wdep[deepest2[v]]) {deepest2[v] = tyt;}} return deepest1[v];}; dfs(root, -1); vector<int> furthest(V); function<void(int, int, D, int)> go = [&](int v, int u, D dst_to_u, int p) -> void {furthest[v] = dst_to_u > wdep[deepest1[v]] - wdep[v] ? u : deepest1[v]; for (const auto& e : g[v]) {if (e.to == p) continue; int nwu = deepest1[e.to] == deepest1[v] ? deepest2[v] : deepest1[v]; D nwd = nwu == -1 ? (D)-1 : wdep[nwu] - wdep[v]; if (nwu == -1 || dst_to_u > wdep[nwu] - wdep[v]) nwu = u, nwd = dst_to_u; go(e.to, nwu, nwd + e.data, v);}}; go(root, -1, (D)-1, -1); return furthest;}
    int64_t calc_sum_of_distances_between_all_pairs_of_vertexes_unweighted() {int64_t o = 0; function<void(int, int)> dfs = [&](int v, int pr) -> void {for (const auto& e : (*this)[v]) {if (e.to == pr) continue; dfs(e.to, v); o += (int64_t)sz[e.to] * (V - sz[e.to]);}}; dfs(root, -1); return o;}
    template<typename D> D calc_sum_of_distances_between_all_pairs_of_vertexes_weighted() {D o = 0; function<void(int, int)> dfs = [&](int v, int pr) -> void {for (const auto& e : (*this)[v]) {if (e.to == pr) continue; dfs(e.to, v); o += (D)e.data * sz[e.to] * (V - sz[e.to]);}}; dfs(root, -1); return o;}
    ...
    vector<int> calc_prufer_code() {assert(is_prepared && V > 1); vector<int> cnt(V), pr(V); function<void(int, int)> dfs = [&](int v, int p) -> void {pr[v] = p; for (const auto& e : g[v]) {if (e.to != p) dfs(e.to, v);}}; dfs(V - 1, -1); for (int q = 0; q < V; ++q) cnt[q] = g[q].size(); vector<int> ans(V - 2); for (int q = 0, leaf = (int)(find(cnt.begin(), cnt.end(), 1) - cnt.begin()), ptr = leaf + 1; q < V - 2; ++q) {int p = pr[leaf]; ans[q] = p; --cnt[leaf]; if (--cnt[p] == 1 && p < ptr) {leaf = p;} else {for (; cnt[ptr] != 1;) ++ptr; leaf = ptr++;}} return ans;}
    ...
    template<typename T> vector<int> calc_maxcost_independent_set(vector<T> cost) {assert(cost.size() == V); vector<array<T, 2>> dp(V); function<void(int, int)> dfs = [&](int v, int p) -> void {dp[v][1] = cost[v]; for (const auto& e : (*this)[v]) {if (e.to == p) continue; dfs(e.to, v); dp[v][1] += dp[e.to][0]; dp[v][0] += max(dp[e.to][0], dp[e.to][1]);}}; dfs(root, -1); vector<int> ans; function<void(int, int, int)> dfs2 = [&](int v, int p, int mode) -> void {if (mode == 1) ans.push_back(v); for (const auto& e : (*this)[v]) {if (e.to == p) continue; dfs2(e.to, v, mode ? 0 : dp[e.to][1] > dp[e.to][0]);}}; dfs2(root, -1, dp[root][1] > dp[root][0]); return ans;}
    ...
    void print() {const int SW = 3; vector<array<int, 2>> dp(V); function<array<int, 2>(int, int)> dfs = [&](int v, int p) {int h = 1, w = 1; for (int nc = 0; const auto& e : (*this)[v]) {if (e.to == p) continue; auto [eh, ew] = dfs(e.to, v); w = nc == 0 ? ew : w + SW + ew; h = nc == 0 ? eh + 2 : max(h, eh + 2); ++nc;} return dp[v] = {h, w};}; auto [H, W] = dfs(root, -1); vector res(H, string(W, ' ')); auto write_num = [&](int x, int y, int v) {string s = to_string(v); if (y + s.size() > W) {W = y + s.size(); for (auto& s : res) s.resize(W, ' ');} for (int i = 0; i < s.size(); ++i) res[x][y + i] = s[i];}; function<int(int, int, int, int, int, int)> build = [&](int v, int p, int lx, int rx, int ly, int ry) {vector<int> yy; for (int sy = 0; const auto& e : (*this)[v]) {if (e.to == p) continue; yy.push_back(build(e.to, v, lx + 2, lx + 2 + dp[e.to][0] - 1, ly + sy, ly + sy + dp[e.to][1] - 1)); sy += dp[e.to][1] + SW;} sort(yy.begin(), yy.end()); if (yy.empty()) {assert(lx == rx && ly == ry); write_num(lx, ly, v); return ly;} int myy = yy[yy.size() / 2]; write_num(lx, myy, v); fill(res[lx + 1].begin() + yy[0], res[lx + 1].begin() + yy.back() + 1, '-'); for (auto y : yy) res[lx + 1][y] = '+'; return myy;}; build(root, -1, 0, res.size() - 1, 0, res[0].size() - 1); for (int i = 0; i < H; ++i) {for (int j = 0; j < W; ++j) {if (res[i][j] == '+' && (j == 0 || res[i][j - 1] == ' ') && (j + 1 == W || res[i][j + 1] == ' ')) {res[i][j] = '|';}}} for (auto& s : res)cout << s << '\n';}
};
