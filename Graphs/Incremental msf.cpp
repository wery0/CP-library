...
class incremental_msf {
    ...
    Node* access_by_pos(Node* n, int ps) {
        for (;;) {
            push(n);
...
public:
    incremental_msf() = default;
    incremental_msf(size_t n): n(n), m(n) {
        for (size_t i = 0; i < n; ++i) {
            m[i] = new Node(0);
        }
    }

    //O(log(n)) amortized with big constant behind asymptotics
    void add_edge(size_t x, size_t y, T w) {
        assert(x < n && y < n);
        Node* nw = new Node(w);
