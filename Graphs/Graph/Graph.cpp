...
public:
    ...
    pair<vector<int>, vector<int>> BFS_from_cluster_on_inverse_graph(vector<int> cluster) {assert(is_prepared); vector<int> dst(V, -1), pr(V, -1); for (int i : cluster) dst[i] = 0; vector<int> s; for (int q = 0; q < V; ++q) if (dst[q] != 0) s.push_back(q); deque<int> dq(cluster.begin(), cluster.end()); vector<bool> tyt(V); while (dq.size()) {int v = dq.front(); dq.pop_front(); for (const auto& e : (*this)[v]) tyt[e.to] = 1; for (int q = 0; q < s.size(); ++q) {const int h = s[q]; if (!tyt[h]) {dst[h] = dst[v] + 1; pr[h] = v; dq.push_back(h); if (q + 1 < s.size()) swap(s[q], s.back()); s.pop_back(); --q;}} for (const auto& e : (*this)[v]) tyt[e.to] = 0;} return {dst, pr};}
