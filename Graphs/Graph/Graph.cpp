...
class graph {
    ...
    vector<int> get_any_cycle_vertex_nums(const bool is_dir) {assert(!(is_dir ? is_acyclic_directed() : is_acyclic_undirected())); vector<int> state(V), ans; auto dfs = [&](auto&& dfs, int v, int p = -1) -> int {state[v] = 1; int ret = -1, cntp = 0; for (const auto& h : (*this)[v]) {if (h.to == p && ++cntp == 1) continue; if (state[h.to] == 1) {if (h.to == v) {ans = {v}; return -2;} return h.to;} int res = dfs(dfs, h.to, is_dir ? -1 : v); if (res == -1) continue; if (res == -2) {ret = -2; break;} ans.push_back(h.to); if (v == res) ans.push_back(v); return v != res ? res : -2;} state[v] = 2; return ret;}; for (int q = 0; q < V && ans.empty(); ++q) if (state[q] == 0) dfs(dfs, q); assert(!ans.empty()); reverse(ans.begin(), ans.end()); return ans;}
    vector<int> get_any_cycle_edge_nums(const bool is_dir) {assert(!(is_dir ? is_acyclic_directed() : is_acyclic_undirected())); vector<int> state(V), ans; auto dfs = [&](auto&& dfs, int v, int p = -1) -> int {state[v] = 1; int ret = -1, cntp = 0; for (const auto& h : (*this)[v]) {if (h.to == p && ++cntp == 1) continue; if (state[h.to] == 1) {if (h.to == v) {ans = {h.data}; return -2;} ans.push_back(h.data); return h.to;} int res = dfs(dfs, h.to, is_dir ? -1 : v); if (res == -1) continue; if (res == -2) {ret = -2; break;} ans.push_back(h.data); return v != res ? res : -2;} state[v] = 2; return ret;}; for (int q = 0; q < V && ans.empty(); ++q) if (state[q] == 0) dfs(dfs, q); assert(!ans.empty()); reverse(ans.begin(), ans.end()); return ans;}
...
public:
    ...
    vector<int> get_any_directed_cycle_as_vertex_nums() {return get_any_cycle_vertex_nums(true);}
    vector<int> get_any_directed_cycle_as_edge_nums() {return get_any_cycle_edge_nums(true);}
    vector<int> get_any_undirected_cycle_as_vertex_nums() {return get_any_cycle_vertex_nums(false);}
    vector<int> get_any_undirected_cycle_as_edge_nums() {return get_any_cycle_edge_nums(false);}
    ...
    vector<int> get_cutpoints() {assert(is_prepared); vector<bool> us(V); vector<int> is_cp(V), tin(V), fup(V); auto dfs = [&](auto&& dfs, int v, int p = -1) -> void {static int T = 0; us[v] = 1; tin[v] = fup[v] = T++; int chd = 0; for (const auto& h : (*this)[v]) {if (h.to == p) continue; if (!us[h.to]) {++chd; dfs(dfs, h.to, v); fup[v] = min(fup[v], fup[h.to]); is_cp[v] |= p == -1 ? chd > 1 : fup[h.to] >= tin[v];} else fup[v] = min(fup[v], tin[h.to]);}}; for (int q = 0; q < V; ++q) if (!us[q]) dfs(dfs, q); vector<int> ans; for (int q = 0; q < V; ++q) if (is_cp[q]) ans.push_back(q); return ans;}
    ...
    vector<int> get_bipartite_coloring() {assert(is_prepared); vector<int> col(V, -1); bool fl = 1; auto dfs = [&](auto&& dfs, int v, int c) -> void {col[v] = c; for (const auto& h : (*this)[v]) {if (col[h.to] == -1) dfs(dfs, h.to, c ^ 1); else fl &= col[h.to] != c;}}; for (int q = 0; q < V && fl; ++q) if (col[q] == -1) dfs(dfs, q, 0); assert(fl && "Graph is not bipartite"); return col;}
    ...
    vector<int> BFS_from_cluster(vector<int> cluster) {assert(is_prepared); vector<int> dst(V, -1); for (int i : cluster) dst[i] = 0; deque<int> dq(cluster.begin(), cluster.end()); while (dq.size()) {int v = dq.front(); dq.pop_front(); for (const auto& e : (*this)[v]) {if (dst[e.to] == -1) {dst[e.to] = dst[v] + 1; dq.push_back(e.to);}}} return dst;}
    vector<int> BFS_from_cluster_on_inverse_graph(vector<int> cluster) {assert(is_prepared); vector<int> dst(V, -1); for (int i : cluster) dst[i] = 0; vector<int> s; for (int q = 0; q < V; ++q) if (dst[q] != 0) s.push_back(q); deque<int> dq(cluster.begin(), cluster.end()); vector<bool> tyt(V); while (dq.size()) {int v = dq.front(); dq.pop_front(); for (const auto& e : (*this)[v]) tyt[e.to] = 1; for (int q = 0; q < s.size(); ++q) {const int h = s[q]; if (!tyt[h]) {dst[h] = dst[v] + 1; dq.push_back(h); if (q + 1 < s.size()) swap(s[q], s.back()); s.pop_back(); --q;}} for (const auto& e : (*this)[v]) tyt[e.to] = 0;} return dst;}
