...
public:
    ...
    vector<int> get_any_undirected_cycle_as_vertex_nums() {assert(!is_acyclic_undirected()); vector<int> state(V), ans; auto dfs = [&](auto&& dfs, int v, int pr) -> int {state[v] = 1; int ret = -1; for (const auto& h : (*this)[v]) {if (h.to == pr) continue; if (state[h.to] == 1) return h.to; int res = dfs(dfs, h.to, v); if (res == -1) continue; if (res == -2) {ret = -2; break;} ans.push_back(h.to); if (v == res) ans.push_back(v); return v != res ? res : -2;} state[v] = 2; return ret;}; for (int q = 0; q < V && ans.empty(); ++q) if (state[q] == 0) dfs(dfs, q, -1); assert(!ans.empty()); reverse(ans.begin(), ans.end()); return ans;}
    vector<int> get_any_undirected_cycle_as_edge_nums() {assert(!is_acyclic_undirected()); vector<int> state(V), ans; auto dfs = [&](auto&& dfs, int v, int pr) -> int {state[v] = 1; int ret = -1; for (const auto& h : (*this)[v]) {if (h.to == pr) continue; if (state[h.to] == 1) {ans.push_back(h.data); return h.to;} int res = dfs(dfs, h.to, v); if (res == -1) continue; if (res == -2) {ret = -2; break;} ans.push_back(h.data); return v != res ? res : -2;} state[v] = 2; return ret;}; for (int q = 0; q < V && ans.empty(); ++q) if (state[q] == 0) dfs(dfs, q, -1); assert(!ans.empty()); reverse(ans.begin(), ans.end()); return ans;}
