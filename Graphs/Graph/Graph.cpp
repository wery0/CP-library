...
class graph {
    ...
    vector<int> get_nums_of_inverse_edges() {assert(is_prepared); vector<int> ans(E, -1); vector<bool> us(V); unordered_map<ll, vector<int>> mp(E); for (int v = 0; v < V; ++v) {for (int i = fir[v]; i < fir[v + 1]; ++i) {const auto& e = store[i]; ll hs = 1ll * v * V + e.to; ll ihs = 1ll * e.to * V + v; if (mp.count(ihs)) {vector<int>& kek = mp[ihs]; int ips = kek.back(); kek.pop_back(); if (kek.empty()) mp.erase(ihs); ans[i] = ips; ans[ips] = i;} else mp[hs].push_back(i);}} assert(mp.empty()); assert(count(ans.begin(), ans.end(), -1) == 0); return ans;}
...
public:
    ...
    void add_edge(int x, Edge e, const bool is_directed) {if (V != -1) assert(0 <= x && x < V), assert(0 <= e.to && e.to < V); acc_edge.emplace_back(x, e); if (!is_directed) {swap(x, e.to); acc_edge.emplace_back(x, e);}}
