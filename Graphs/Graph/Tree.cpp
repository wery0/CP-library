...
public:
    ...
    int calc_diameter_unweighted() {assert(is_prepared); int v = max_element(all(dep)) - dep.begin(); auto dfs = [&](auto&& dfs, int v, int p) -> int{int mxd = -1; for (const auto& e : g[v]) {if (e.to != p) {int tyt = dfs(dfs, e.to, v); mxd = max(mxd, tyt);}} return mxd + 1;}; return dfs(dfs, v, -1);}
