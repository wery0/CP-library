/*
Maximum in general graphs
- O(msqrt(n)log_{max(2, 1 + m/n)}(n)) time, really fast on practice (takes seconds on 1e6 vertex graphs)
- O(n + m) space

Reference: Harold N. Gabow, "The Weighted Matching Approach to Maximum Cardinality Matching" (2017) (https://arxiv.org/abs/1703.03998)
*/
class max_general_matching_gabow {
	...
	struct DisjointSetUnion {
		...
		DisjointSetUnion(int N): par(N) {iota(par.begin(), par.end(), 0);}
...
public:
	max_general_matching_gabow(int N, const vector<array<int, 2>>& in): N(N), NH(N >> 1), ofs(N + 2, 0), edges(in.size() * 2) {
		for (auto& e : in) ofs[e[0] + 2] += 1, ofs[e[1] + 2] += 1;
		for (int i = 1; i <= N + 1; ++i) ofs[i] += ofs[i - 1];
		for (auto& e : in) {
			edges[ofs[e[0] + 1]++] = {e[0] + 1, e[1] + 1};
			edges[ofs[e[1] + 1]++] = {e[1] + 1, e[0] + 1};
		}
	...
	vector<array<int, 2>> calc_max_matching() {
		...
		vector<array<int, 2>> ans; ans.reserve(match);
		for (int i = 1; i <= N; ++i) {
			if (mate[i] > i) ans.push_back({i - 1, mate[i] - 1});
		}
		return ans;
	}
};
