...
auto kth_ordered_statistic_of_two_sorted_arrays(const T_arr& m1, const T_arr& m2, int k) {
    ...
    function<array<int, 2>(int, int, int, int, int, int)> go = [&](int u, int i1, int n1, int i2, int n2, int k) -> array<int, 2> {
        if (n1 > n2) return go(u ^ 1, i2, n2, i1, n1, k);
        ...
        return fl ? go(u, i1 + i, n1 - i, i2, n2, k - i) :
               go(u, i1, n1, i2 + j, n2 - j, k - j);
    };
    assert(k < m1.size() + m2.size());
    auto [u, i] = go(0, 0, m1.size(), 0, m2.size(), k);
    return (u == 0 ? m1 : m2)[i];
}
