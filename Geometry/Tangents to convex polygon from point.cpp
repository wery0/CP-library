...
template<typename T>
pair<int, int> get_tangents_to_convex_polygon_from_point(const vector<pt<T>>& ch, const pt<T>& p,
        bool furthest_points = false, pair<int, int> location = {-3, -3}) {
    ...
    auto go = [&](size_t l, size_t r) -> size_t {
